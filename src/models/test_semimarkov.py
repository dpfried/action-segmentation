...
def synthetic_data(num_data_points=200, C=3, N=100, K=5):
    def make_synthetic_features(class_labels, shift_constant=5.0):
...
def test_learn_synthetic():
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            true_parts = SemiMarkovCRF.struct.to_parts(model.add_eos(spans, lengths), (C+1, K), lengths=lengths+1).type_as(scores)
            this_loss = -dist.log_prob(true_parts).mean()
            this_loss.backward()
...
test_labels_to_spans()
print("test_labels_to_spans passed")

test_log_hsmm()
print("test_log_hsmm passed")

model = test_learn_synthetic()
